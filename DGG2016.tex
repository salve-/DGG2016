...
\textbf{-1.5 to +2.3 \%}
            			\end{small}
					\end{tcolorbox}};
			\draw [<->,>=stealth,green,ultra thick,sloped] (box1)-- node[above] {\color{black}\textbf{$\ll$}} ++(box3);
...
\begin{frame}
 ...
 $\Rightarrow$ Apparent delay of max.											\textbf{0.52 \%} for extreme shifts in corner frequency.
			...
			\node[anchor=south west,inner sep=0] (box3) at (2.7,-1.2) {
					...
					\end{tcolorbox}};
			\draw [<->,>=stealth,green,ultra thick,sloped] (box1)-- node[above] {\color{black}\textbf{$\ll$}} ++(box3);
			\draw [<->,>=stealth,green,ultra thick,sloped] (box2)-- node[below] {\color{black}\textbf{$<$}} ++(box3) ;
        \end{tikzpicture}
   \end{center}
\end{frame}
...
\begin{frame}
	\begin{tikzpicture}[overlay]
		\node[anchor=south west,inner sep=0] (image) at (0,1.5) 	{\includegraphics[width=0.95\linewidth]{Figures/BAM_bridge.pdf}};
		\node[anchor=south west,inner sep=0] (image) at (0.5,-2.5) 	{\includegraphics[width=0.6\linewidth]{Figures/TU-plate2.jpg}}; 
		\node[anchor=south west,inner sep=0] (image) at (0,-4.3) 	{\includegraphics[width=0.7\linewidth]{Figures/bridge3.png}}; 
		\node[anchor=south west,inner sep=0] (image) at (8.5,-3.7) 	{\includegraphics[width=0.3\linewidth]{Figures/abriss.jpg}};
	\end{tikzpicture}
\end{frame}
